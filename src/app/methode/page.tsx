...
export default function MethodePage() { return <>
  ...
  <section className="section section--ink"><Container><SectionHeading eyebrow="Sept principes" title="Rendre la complexité maîtrisable" light /><div className="principle-grid">{methode.principles.map((principle, index) => <article key={principle}><span>0{index + 1}</span><h3>{principle}</h3></article>)}</div></Container></section>
  ...
  <CTASection text="Évaluez la méthode adaptée à votre projet avec Archeritage." />
  </>; }
