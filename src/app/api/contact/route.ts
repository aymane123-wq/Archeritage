...
export async function POST(request: Request) {
  ...
  if (
    ...
    && attachment.size > 0
    && (attachment.size > MAX_FILE_SIZE || !ALLOWED_TYPES.has(attachment.type))
  ) {
    return NextResponse.json({ message: 'La pièce jointe doit être un PDF, DOC, DOCX, JPG ou PNG de 5 Mo maximum.' }, { status: 400 });
  }
  ...
  return NextResponse.json({ message: 'Votre demande a bien été transmise. Archeritage vous répondra sous 48 heures ouvrées.' });
}
